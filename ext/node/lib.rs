...
deno_core::extension!(deno_node,
  ...
  lazy_loaded_esm = [
    ...
    // trex: node:http server implementation replaced wholesale by the fork's
    // `_http_server.ts` (Deno.serve-backed). Upstream's `_http_server.js` is
    // left in the tree but is no longer registered.
