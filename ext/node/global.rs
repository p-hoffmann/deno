...
// ext/node changes the global object to be a proxy object that intercepts
// property accesses for globals that must differ between Node and Deno,
// dynamically resolving them against a different backing object depending on
// whether the accessing code lives in node_modules/ or not. The distinction is
// carried by the script's host-defined options: the module loader tags every
// module it resolves inside an npm package (see
// `create_host_defined_options`), and `wrapSafe` in 01_require.js tags every
// CJS file it compiles.
//
// To make this performant, a v8 named property handler is used. It is
// non-masking, so it is only consulted for properties that are not already
// present on the global object; in the common case, when code accesses a
// global that is the same in both worlds (like Uint8Array or fetch), the proxy
// overhead is avoided.
//
// The masking of a managed global happens through the `definer` hook rather
// than the `getter`: when Deno-side bootstrap code defines one of these names
// on `globalThis`, the definition is captured into the Deno bag instead of
// landing on the global object, so Node-side code never finds it. Deno-side
// reads are then served back out of that bag by the `getter`.
//
// The Deno and Node specific globals are stored in a struct in a context slot.
//
// Upstream deleted this file in denoland/deno#33249 along with the Node/Deno
// timer split, and along with it the `clearInterval`/`clearTimeout`/`process`/
// `setInterval`/`setTimeout` entries this list used to carry. Those five are
// deliberately NOT restored: #33249 made the global timers the Node ones and
// stopped populating the Node bag from 02_init.js, so managing them here would
// capture them into the Deno bag and leave npm code with no `setTimeout` or
// `process` at all. Only `window` is restored, because that is the one entry
// whose masking an embedder still depends on -- a runtime that declares a
// `window` global for its own user code must not hand it to npm packages that
// sniff `typeof window === "object"` to pick a browser code path.

// UTF-16 encodings of the managed globals. THIS LIST MUST BE SORTED.
#[rustfmt::skip]
const MANAGED_GLOBALS: [&[u16]; 1] = [
